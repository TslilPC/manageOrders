...
class ManageRestaurant(object):
    ...
    def start_manage(self):
        ...
        self._dough_to_toppings_pipe.manage()
        time.sleep(RestaurantConsts.DOUGH_DELAY)
        self._toppings_to_oven_pipe.manage()
        time.sleep(RestaurantConsts.TOPPING_DELAY)
        self._oven_to_waiters_pipe.manage()
        time.sleep(RestaurantConsts.OVEN_DELAY)
        self._waiters_pipe.manage()
    ...
    def _set_pipes(self):
        ...
        self._orders_out_queue = multiprocessing.Queue() #waiters out

        self._dough_to_toppings_pipe = DoughPipe(self.in_orders_queue, topping_in_queue,
                                                RestaurantConsts.DOUGH_CHEFS_NUM, RestaurantConsts.DOUGH_DELAY)
        self._toppings_to_oven_pipe = ToppingsPipe(topping_in_queue, oven_in_queue, RestaurantConsts.TOPPINGS_CHEFS_NUM,
                                                  RestaurantConsts.TOPPING_DELAY, RestaurantConsts.TOPPINGS_PER_WORKER)
        self._oven_to_waiters_pipe = OvenPipe(oven_in_queue, waiters_in_queue, RestaurantConsts.OVENS_NUM,
                                             RestaurantConsts.OVEN_DELAY)
        self._waiters_pipe = WaitersPipe(waiters_in_queue, self._orders_out_queue, RestaurantConsts.WAITERS_NUM,
                                        RestaurantConsts.DOUGH_DELAY)

    def stop_manage(self):
        tot = len(self.orders)
        while True:
            for name, _ in iter(self._orders_out_queue.get, None):
                ...
                if tot == 0:
                    self._stop_pipes()
                    self._update_orders_at_report()
                    return



    def _update_orders_at_report(self):
    ...
    def _stop_pipes(self):
        self._dough_to_toppings_pipe.stop()
        self._toppings_to_oven_pipe.stop()
        self._oven_to_waiters_pipe.stop()
        self._waiters_pipe.stop()
